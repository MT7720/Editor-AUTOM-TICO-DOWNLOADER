...
def _attempt_translate_text(
    ...
    translator_source = LANGUAGE_TRANSLATION_CODES.get(source_code) if source_code else None

    # Algumas APIs de tradução falham com textos muito longos. Cortamos em blocos
    # menores para minimizar erros e possibilitar "retry" em partes específicas.
    def _iter_chunks(value: str) -> List[str]:
        ...
        for paragraph in value.split('\n\n'):
            ...
            if not paragraph:
                blocks.append('')
                continue
            while len(paragraph) > 4500:
                blocks.append(paragraph[:4500])
                paragraph = paragraph[4500:]
            blocks.append(paragraph)
        return blocks or ['']

    def _translate_with_google(chunks: List[str]) -> Optional[str]:
        if GoogleTranslator is None:
            return None
        try:
            translated_chunks: List[str] = []
            translator = GoogleTranslator(source=translator_source or "auto", target=translator_target)
            for piece in chunks:
                translated_piece = translator.translate(piece) if piece else ''
                translated_chunks.append((translated_piece or '').strip())
            return '\n\n'.join(translated_chunks)
        except Exception as exc:  # pragma: no cover - depende de serviço externo
            logger.warning("Falha ao traduzir texto via GoogleTranslator para %s: %s", target_code, exc)
        return None

    def _translate_with_mymemory(chunks: List[str]) -> Optional[str]:
        if MyMemoryTranslator is None:
            return None
        try:
            translated_chunks: List[str] = []
            translator = MyMemoryTranslator(
                source=translator_source or 'auto',
            ...
            logger.warning("Falha ao traduzir texto via MyMemoryTranslator para %s: %s", target_code, exc)
        return None

    chunks = _iter_chunks(text)
    attempts: List[Callable[[List[str]], Optional[str]]] = [
        _translate_with_google,
        _translate_with_mymemory,
    ]

    for attempt in attempts:
        translated_text = attempt(chunks)
        if translated_text:
...
def _prepare_intro_text(
    ...
    if selected_language and selected_language in intro_texts:
        ...
        if selected_language:
            translated, translation_applied = _attempt_translate_text(
                base_text,
                selected_language,
                base_language_code,
            )
